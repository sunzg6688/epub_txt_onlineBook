...
{ 
	...
	public final class EPub extends EventDispatcher {
		...
		public static var _images:Object;
...
//			if(_pageIndex>5)return;
			...
			_processingSections = XhtmlToTextFlow.splitIntoSections2(flatPage);
//			XhtmlToTextFlow.splitIntoSections2(flatPage);
//			_processingSections = XhtmlToTextFlow.sections;
...
//			if(_pageIndex>400)return;
			if(XhtmlToTextFlow.isGo){
				_processingSections = XhtmlToTextFlow.splitIntoSection3();
			}
//			if(itemIndex>=3){
//				if(XhtmlToTextFlow.isGo){
//					_processingSections = XhtmlToTextFlow.splitIntoSection3();
...
//			}
		...
		protected function processTocFromNcx():void {
			...
			for (var i:int=0; i<items.length(); i++) {
				...
				p.page = _spine.getPageByPath(c);//每个章节的 第一页
