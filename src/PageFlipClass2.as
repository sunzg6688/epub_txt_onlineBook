...
package 
{
	...
	public class PageFlipClass2 {
		...
		public var limit_point1:Point;
		public var limit_point2:Point;
		...
		public var bookArr:Array;
		
		public function resetBook():void{
			if(!bookArr)return;
			for (var i:Number = 1; i<=bookArr.length; i++) {
				if(book_root["pload_"+i]){
					book_root["pload_"+i]=null;
				}
			}
			if(pageMC.numChildren!=0){
				this.pageMC.removeChildAt(0);
			}
			if(bgMC.numChildren!=0){
				this.bgMC.removeChildAt(0);
			}
			if(book_root.numChildren!=0){
				book_root.removeChildAt(0);
			}
		}
		//**init Parts------------------------------------------------------------------------
		public function InitBook():void {
//			book_page=book_topage=book_initpage;
			book_topage=book_page;
...
//			SetPageMC(7);
			if(!book_timer){
				...
				FlexGlobals.topLevelApplication.addEventListener(MouseEvent.MOUSE_DOWN, MouseOnDown);
				FlexGlobals.topLevelApplication.addEventListener(MouseEvent.MOUSE_UP, MouseOnUp);
		...
		public function setLoadMcPrev():void{
			for (var i:Number = 1; i<=bookArr.length; i++) {
				...
				book_root["pload_" + i].addChild(bookArr[i-1] as Sprite);
			}
		}
		
		public function SetLoadMCNext():void {
//			nowLoadIndex = 1;
//			loadIndexArray = new Array();
			for (var i:Number = 1; i<=bookArr.length; i++) {
				if(book_root["pload_"+i])continue;
				book_root["pload_" + i]=new MovieClip();
				book_root["pload_" + i].id=i;
				book_root["pload_" + i].loadedflag=false;
				book_root["pload_" + i].loadedtype="";
...
//			loadPageFile();
		...
		private function createPreLoadingMC(targetPage:MovieClip):void
		{
			targetPage.graphics.beginFill(0x333333);
		...
		private function loadPageFile():void
		{
		...
		public function SetPageMC(pageNum:Number):void {
...
//			trace("**************");
		...
		public var listMouseDown:Boolean=false;
		//**MouseEvent Parts------------------------------------------------------------------------
		private function MouseOnDown(evt:Event):void {
			...
			if (book_TimerArg0==0) {
				...
				//向右翻到顶
				return;
			}else if((book_TimerArg0==3||book_TimerArg0==4)&&book_page+1>=book_totalpage){
				return;//最后一页
			}else {
...
//			onPageStart();
			...
			if (book_TimerArg0==1||book_TimerArg0==2) {
				...
				page3=book_topage;
				page4=book_page+1;
//				trace("timerArg::::",book_TimerArg0,"::::",page1,page2,page3,page4);
